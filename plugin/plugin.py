...
class xpower(Screen, HelpableScreen):
	...
	def buildPCViewItem(self, entry):
                pc = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, plugin_path+"/img/host.png"))
                name = entry["name"]
		ip = _("IP:") + " " + str(entry["ip"])
                mac = _("MAC:") + " " + str(entry["mac"])
		system = entry["system"]
		if system == OS_WIN7:
			logo = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, plugin_path+"/img/win.png"))
		elif system == OS_LINUX:
			logo = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, plugin_path+"/img/lin.png"))
		elif system == OS_RPC:
			logo = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, plugin_path+"/img/rpc.png"))
		else:
			logo = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, plugin_path+"/img/xp.png"))
	...
	def help(self):
		self.session.open(xpowerHelp, plugin_path)
...
def xpowerMain(session, **kwargs):
	session.open(xpower, plugin_path)
